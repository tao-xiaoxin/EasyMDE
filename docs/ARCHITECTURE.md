...
Each article-theme descriptor exposes a Registry-owned `defaultCodeTheme`. Legacy article themes continue to share `atom-one-dark`, while each selected light Typora variant has a distinct `*-code` association in the independent `CodeThemeRegistry`; those palettes are scoped together in `assets/themes/code/typora-derived.css` and never own the Mac frame geometry. `fullstack-blue` retains its genuinely distinct token palette without content-dependent JavaScript rewriting. When no valid persisted or browser-session code-theme choice exists, the current article association supplies the code theme. A valid explicit choice remains authoritative across later article-theme changes. `atom-one-dark` is only the compatibility fallback for a missing or invalid association, and opening the editor performs no hidden write.
