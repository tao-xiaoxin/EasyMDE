...
当前 React 官方站点可能默认展示 React 19。EasyMDE 的运行时结论必须同时满足 WordPress 6.7 的实际源码。WordPress 6.7 使用 React 与 ReactDOM 18.3.1，并通过 `@wordpress/element` 暴露运行时；提高最低 WordPress 版本前必须重新核对对应 Tag，不能把最新 Gutenberg 文档直接当成当前 Core 能力。
...
- **App Provider**：把 Runtime、Store、翻译或错误边界放入 React 树的组件；不是后端 Adapter。
- **Controller Hook**：复杂 Feature 的 Headless 行为层。
- **View**：由类型化状态和动作驱动的 UI。
- **Store**：一个应用 Root 的浏览器会话状态 Owner。
...
Editor 和 Settings 可共享纯 Domain、Contracts 和真正通用的 UI Primitive，但不能共享可变 Store、Query Cache 或生命周期 Owner。
...
- `entrypoints/`：定位 Root、解析 Bootstrap、组装 Runtime 和 Store、Mount、Ready、Teardown。
- `app/`：Root Shell、Providers、Error Boundary、Store 和顶层组合。
...
- `shared/`：至少两个稳定消费者真正共享、且没有 Feature 或 WordPress 所有权的代码。
...
Feature 不导入具体 Adapter；Entrypoint 负责组装。禁止循环依赖、向上导入、跨 Feature 私有深层导入和通过 Barrel 隐藏依赖。
...
React-owned State 保持在 React / Root Store。读取外部可变 State 时使用项目 Hook 包装 `useSyncExternalStore`：`subscribe` 必须稳定并返回幂等 Cleanup；`getSnapshot` 返回 Immutable Snapshot，并在底层值未变化时复用同一对象；消费者只订阅最小 Slice。禁止轮询、每次返回新 Snapshot、Render 中直接读取后期待自动更新，或用 Effect 镜像到另一份 State。
...
当 React 使用 `@wordpress/i18n` 时，同一变更必须：

- 使用 `easymde` Text Domain 和 `__`、`_x`、`_n`、`sprintf` 等 WordPress API；
- 扩展 TS / TSX 提取并生成、校验 JavaScript JSON Catalog；
- 为 Classic Script 声明 `wp-i18n`，先注册 Handle 再调用 `wp_set_script_translations()`；
- 对 Script Module 单独验证 WordPress 6.7 的翻译加载合同，不假定 Classic API 自动适用；
- 把 Catalog 纳入 Release ZIP 和 i18n Check；
- 验证 Placeholder、Context、Plural、Long Translation 和 RTL。

同一字符串不得同时由 PHP Bootstrap 和 JS Catalog 翻译。不得拼接翻译片段。Error Code、ID、Route、Handle、Storage Key 和扩展标识符不翻译。

旧 JavaScript 中未翻译的 Fallback Literal 只是兼容行为，不是第二翻译权威，也不是 React 实现范式。缺少必需 Bootstrap 文案时应由合同验证明确失败或进入不依赖新英文文案的受控降级状态，不得静默显示新的英文 Fallback。
...
- Source ZIP / tar.gz 由 `scripts/build-source-archives.mjs` 从准确的已跟踪 Commit 生成，包含已跟踪的 `frontend/` Source 与构建、维护文档，并拒绝 Generated 或 Local-only Path。
...
- `app`：Root Provider、Store、Error Boundary、Activation 和 Teardown；
