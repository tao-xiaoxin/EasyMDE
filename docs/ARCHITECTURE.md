...
- `frontend/`: strict TypeScript, React, CodeMirror, and Vite source for the production normal-editor Toolbar, document session, and Preview Request Session, plus the test-only WordPress React build-contract fixture.
...
The Preview Request Session bridge owns normal-editor Preview scheduling after readiness. It preserves the 180 millisecond debounce, aborts superseded browser requests, assigns request revisions, and publishes only the latest response whose revision and Markdown signature still match the active session. Its WordPress Adapter calls the existing protected `easymde/v1` Preview Route with the current bootstrap Nonce; PHP remains the rendering, permission, sanitization, and response authority. Startup failure leaves the legacy scheduler active, while teardown aborts pending work and returns explicit ownership to legacy for a clean lifecycle. The legacy runtime intentionally still owns Preview DOM rendering and enhancement, feature asset loading, scroll restoration, error presentation, and all immersive Preview requests. Normal and immersive request revisions are isolated so the two surfaces cannot invalidate each other.

`AdminAssets` reads `assets/build/wordpress-manifest.json` and the matching dependency metadata before enqueueing the stable `easymde-admin-editor-toolbar` handle ahead of `easymde-admin`. Legacy Toolbar and source owners stay visible during React preparation and mount. Only validated readiness callbacks switch each explicit owner and visibility. A missing, invalid, or failed production entry leaves the corresponding legacy owner usable and reports a stable privacy-safe diagnostic. Teardown flushes the submission bridge, unmounts the React Root, and only then returns the container to legacy ownership.
