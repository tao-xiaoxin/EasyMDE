...
EasyMDE 是一个独立的 WordPress Markdown 编辑器插件。它只接管明确选择使用 EasyMDE 的文章或页面，让作者用 Markdown 源文档写作、在右侧实时预览，并继续使用 WordPress 原生的媒体库、修订版本、权限和发布流程。

普通文章和页面不会被全局改成 EasyMDE：通过 WordPress 默认入口新建的内容仍然使用 Gutenberg。EasyMDE 适合那些你希望长期保留 Markdown 源文档、同时又需要 WordPress HTML 兼容输出的文章。
...
3. 需要写 Markdown 时，使用 **Posts > Add EasyMDE Post** 或 **Pages > Add EasyMDE Page** 入口创建内容。
4. 已经带有 EasyMDE Markdown 元数据的旧文章会继续用 EasyMDE 打开。

如果你从 WordPress 默认的“写文章”或“新建页面”入口进入，编辑器仍然是 Gutenberg。
