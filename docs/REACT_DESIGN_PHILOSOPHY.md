...
Focus Mode / 沉浸式写作不属于普通编辑器默认表面；Issue #126 允许在同一
React Root 中打开沉浸式呈现，但必须复用现有文档、Preview、原生表单和
WordPress 能力所有者，不得创建第二个根、编辑器、渲染器或保存链路。
沉浸界面可以投影 Publish、Revision、分类、标签、摘要和特色图片交互，
但只能读取和更新现有 WordPress 原生字段、调用现有媒体与修订能力，并把
最终提交或恢复交回 WordPress；它不得成为第二套权限、持久化或修订所有者。
...
组件能力按固定顺序选择：先复用 EasyMDE 或 WordPress 已有能力，再使用浏览器完整实现的原生语义控件；复杂 Widget 采用满足 Focus、Keyboard、ARIA、Portal、定位和生命周期合同的单项 Headless Primitive；只有前三者无法满足已验证需求时才实现项目组件。具体候选与依赖审查由
`.agents/skills/easymde/SKILL.md` 负责，本文不维护会随生态变化的 Package 清单。
...
- React Query、React Hook Form 或 Material UI；
...
- Active React Container 由对应 Root 独占；其他 Runtime 不得删除、替换或写入，并只在 `root.unmount()` 与 Cleanup 完成后释放；
...
不引入 Router。WordPress 负责页面导航；Tab、Panel 和 Dialog 使用本地 UI State，不创建浏览器路由。
...
视觉还原是可验证的界面合同，不是实现结束后的主观润色。参考源码表达
设计意图和所有权，受控浏览器渲染表达 CSS Cascade、字体、资源、运行时
状态和浏览器默认值共同产生的有效结果；只检查其中一方都不能证明还原
正确。

完整性必须双向证明：参考界面的每个可见区域、控件、图标、标签、分隔线、
Overlay 和行为状态都在目标中有明确对应，目标中每个额外可见元素也有
明确依据。缺失项和无依据的新增项都是合同差异，不能因为尺寸小、状态短暂
或整体截图接近而忽略。WordPress 兼容性、无障碍或当前明确需求确实要求
偏离参考时，保留最小必要差异并记录理由和受保护范围。

实际执行采用相同输入与浏览器条件下的源码映射、几何与 Computed Style
测量、交互和无障碍状态验证，并从第一个发生差异的 Owner 或 Ancestor 修复
根因。完整七阶段流程、持久化证据和隐私边界由
`.agents/skills/easymde/SKILL.md` 负责。
