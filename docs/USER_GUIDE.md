...
The normal WordPress edit screen remains unchanged until the author selects **Enter immersive writing**. That action opens an isolated full-screen article workspace with title editing, edit/split/preview modes, outline navigation, local statistics, responsive panes, themes, fonts, media, revision history, and native WordPress save and publish controls. Immersive writing starts in **Edit**. **Split** keeps Markdown on the left and a read-only preview on the right. **Preview** opens a structured visual Markdown surface and shows an **Editable** / **Read-only** lock while keeping the formatting toolbar available. The lock lasts only for the current workspace session and is not saved with the post or user account.

Editable Preview supports paragraphs, headings, emphasis, strikethrough, inline code, links, blockquotes, ordered and unordered lists, nested and task lists, fenced code, horizontal rules, and visual-session undo/redo. Images, tables, Mermaid, math, TOC markers, raw HTML, and unknown extension syntax appear as protected blocks so their original Markdown cannot be silently rewritten; use **Edit in Markdown** on a protected block to change its source. Pasting rich content uses safe plain text. Pasting or dropping a local image reuses the WordPress media upload flow and returns to Edit to insert a protected Markdown image block. Read-only Preview still allows selection, copying, scrolling, and outline navigation, but blocks typing, destructive keys, paste, drop, formatting, undo, and redo.

Outline navigation uses structural heading identities rather than heading text, so repeated heading labels navigate to the correct occurrence in Edit, Split, Editable Preview, and Read-only Preview without changing the document or lock.

Outline navigation and the compact statistics panel are off by default; the statistics panel computes line, Western-word, CJK-character, total-character, and deterministic reading-time values locally without sending or storing article content.
