...
const fixture = readJson('tests/e2e/fixtures/editor-phase-0.json');
...
function createUser() {
  const slug = `visual-${randomUUID().slice(0, 12)}`;
...
function deleteUserContent(user) {
  const postIds = runWp([
    'post',
    'list',
    `--author=${user.id}`,
  ...
  runWp(['user', 'delete', user.id, '--yes', '--reassign=1']);
}
...
test.describe('EasyMDE fixed Legacy Reference visual capture', () => {
  ...
  test('captures the Phase 0 baseline and writes a privacy-safe manifest', async ({ browser, page }, testInfo) => {
    ...
    const artifactRoot = testInfo.outputPath('visual-evidence');
    const captureRoot = join(artifactRoot, 'captures');
      ...
      await page.evaluate(async () => {
        ...
        await Promise.all(Array.from(document.images).map(async (image) => {
          ...
          if (image.naturalWidth === 0) {
            throw new Error('A rendered image has no decoded pixels.');
          }
        }));
      });
