...
Yes. An authorized article paste or drop contacts the configured provider only after an administrator enables Image Hosting; an administrator's explicit Verify Upload may contact the selected provider regardless of whether `imageHostingEnabled` is true or false, including while pre-configuring it. With Image Hosting disabled, local paste and drag-and-drop use the WordPress Media Library owner and remote image import is off. Verify Upload sends the bundled EasyMDE PNG using the current file-name rule and displays its path and URL. See **External Services** for the exact trigger, transmitted fields, provider links, terms, privacy policies, and failure behavior.
