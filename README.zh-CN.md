...
- 从剪贴板粘贴或拖放本地图片时，只会通过受保护的同源图床上传路径发送到已配置的 Cloudflare R2；该路径不会切换或回退到 WordPress 媒体库。
...
- 本地图片粘贴和拖放通过受保护的同源图床路径上传到已配置的 Cloudflare R2，并可按需启用七牛 Kodo 同路径备份；该路径不提供 WordPress 媒体库回退。
- 本地草稿恢复和独立沉浸写作工作台。
