...
function phpStringKeys(source) {
	return new Set(
		[...source.matchAll(/'([A-Za-z][A-Za-z0-9]*)'\s*=>/g)].map(
			(match) => match[1],
		),
	);
}
...
function jsAdminStringKeys() {
	...
	collectFiles("assets/js/admin", (file) => file.endsWith(".js")).forEach(
		(file) => {
			const source = readFileSync(file, "utf8");
			for (const match of source.matchAll(
				/getString\(\s*['"]([A-Za-z][A-Za-z0-9]*)['"]/g,
			)) {
				keys.add(match[1]);
			}
		},
...
function phpTranslationCallPattern() {
	...
	return new RegExp(
		`(?<![A-Za-z0-9_])(${translationFunctions.join("|")})\\s*\\((${argumentChunk})\\)`,
		"gs",
	);
}
...
function phpStringArguments(source) {
	return [...source.matchAll(/'(?:\\.|[^'\\])*'|"(?:\\.|[^"\\])*"/g)].map(
		(match) => decodePhpStringLiteral(match[0]),
	);
}
...
test("legacy runtime translations are not loaded from production code", () => {
	assert.equal(
		existsSync(join(repoRoot, "src/Support/LegacyTranslations.php")),
		false,
	);
...
test("PHP translation domain check matches real gettext calls", () => {
	...
	const matches = [...source.matchAll(phpTranslationCallPattern())].map(
...
test("PHP-injected string config covers JavaScript user-facing string reads", () => {
	const adminKeys = phpStringKeys(
		sourceSlice("src/Admin/AdminAssets.php", "private function get_strings"),
	);
	const frontendKeys = phpStringKeys(
		sourceSlice("src/Frontend/FrontendAssets.php", "'strings'"),
	);
...
test("default toolbar labels do not trigger gettext during plugin construction", () => {
	...
	const toolbarDefaults = sourceSlice(
	...
	assert.match(
		mainFile,
		/add_action\(\s*'init'\s*,\s*array\(\s*'EasyMDE_Plugin'\s*,\s*'init'\s*\)\s*\)/,
	);
	assert.doesNotMatch(
		mainFile,
		/add_action\(\s*'plugins_loaded'\s*,\s*array\(\s*'EasyMDE_Plugin'\s*,\s*'init'\s*\)\s*\)/,
	);
...
test("WordPress i18n verifier reloads text domains compatibly", () => {
	const verifier = readFileSync(
		join(repoRoot, "scripts/verify-wordpress-i18n.php"),
		"utf8",
	);
	const runtimeChecks = sourceSlice(
		"scripts/verify-wordpress-i18n.php",
		"if ( ! defined( 'ABSPATH' ) )",
	);
	...
	assert.match(
		runtimeChecks,
		/easymde_verify_i18n_unload_for_reload\(\s*'easymde'\s*\)/,
	);
});
...
test("gettext catalog files are current and contain real zh_CN translations", () => {
	...
	const result = spawnSync(
		"msgunfmt",
		[join(repoRoot, "languages/easymde-zh_CN.mo")],
		{
			encoding: "utf8",
		},
	);
...
test("React immersive counters ship one handle-based WordPress Jed catalog", () => {
	...
	const messages = catalog.locale_data?.easymde;

	assert.equal(catalog.domain, "easymde");
	assert.equal(messages?.[""]?.lang, "zh_CN");
	assert.equal(messages?.[""]?.["plural-forms"], "nplurals=1; plural=0;");
	assert.deepEqual(messages?.["%s word"], ["%s 个词"]);
	assert.deepEqual(messages?.["%s character"], ["%s 个字符"]);
	assert.deepEqual(messages?.["About %s minute"], ["约 %s 分钟"]);
	assert.deepEqual(messages?.["%s revision"], ["%s 个修订版本"]);
});
...
"PO-Revision-Date: 2026-07-26 00:00+0000\\n"
"Language: zh_CN\\n"
"MIME-Version: 1.0\\n"
...
`,
		...
		assert.throws(() => checkI18n({ root }), /fuzzy: Shortcut settings/);
	} finally {
		rmSync(root, { recursive: true, force: true });
	}
});
