...
The plugin clones the rendered preview and inlines important computed styles for typography, code blocks, tables, and images before copying. Optional diagram-and-formula PNG conversion is disabled by default. When enabled, an explicit Copy converts only outermost rendered Mermaid diagrams and formulas to PNG; normal tables remain HTML, while existing images, ordinary SVG, code, other media, and unknown content keep their existing rich-text form. Generated PNGs use Image Hosting when enabled or the WordPress Media Library when disabled. No conversion upload runs during preview or background preparation. When supported by the browser, the copied content can be pasted directly into the WeChat Official Accounts editor.
...
Yes. An authorized article paste or drop contacts the configured provider only after an administrator enables Image Hosting. When the administrator also enables diagram-and-formula PNG conversion, an authorized author's explicit ordinary or immersive Copy sends the generated PNG bytes through the selected provider; it does not send Markdown, article HTML, or article text. An administrator's explicit Verify Upload may contact the selected provider regardless of whether `imageHostingEnabled` is true or false, including while pre-configuring it. With Image Hosting disabled, local paste/drop and generated Copy PNGs use the WordPress Media Library owner, and remote image import is off. Verify Upload sends the bundled EasyMDE PNG using the current file-name rule and displays its path and URL. See **External Services** for the exact trigger, transmitted fields, provider links, terms, privacy policies, and failure behavior.

= Can I copy an article into the WeChat Official Accounts editor? =

Yes. Use the **Copy to WeChat** action in the editor. EasyMDE copies the current rendered preview as rich text when browser clipboard support is available. Optional diagram-and-formula PNG conversion is disabled by default. When enabled, Mermaid and formulas use the selected upload owner while normal tables remain HTML; the copy does not change the article or live preview.
...
* Add explicitly triggered uploads to Cloudflare R2, Qiniu Kodo, Alibaba Cloud OSS, and Tencent Cloud COS, with optional backup destinations and bounded retries; exhausted destinations fail visibly without silent provider switching or URL insertion.
