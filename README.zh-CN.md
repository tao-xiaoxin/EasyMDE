...
- 从剪贴板粘贴或拖放本地图片时，只会通过受保护的同源图床上传路径发送到管理员配置的 Cloudflare R2、七牛云 Kodo、阿里云 OSS 或腾讯云 COS；任一服务均可作为主图床或可选备份图床，主备写入固定使用同一个对象 Key，主配置的查看图片域名负责生成返回的公开 URL。管理员配置一个 `0` 至 `5` 的额外重试次数，同时用于主图床和已启用的备份图床串行写入；任一必需目标耗尽尝试后整次上传失败且不插入 URL，并且不会切换服务或回退到 WordPress 媒体库。
- 只有管理员可以执行“验证上传”：WordPress 会按当前文件名规则把插件内置的 EasyMDE PNG 上传到所选图床，并在结构化消息弹窗中显示真实对象路径和文章图片 URL；该操作不会自动重试或切换供应商。上传 Endpoint 必须使用 HTTPS，查看图片域名可使用 HTTP 或 HTTPS。HTTP 图片链接在 HTTPS 文章页面中可能被浏览器作为混合内容拦截，但这不代表上传失败。使用时间或 UUID 变量的规则可能在每次验证时创建新对象。
- 普通设置读取和导出始终隐藏已保存密钥。管理员点击对应眼睛后，可以通过受保护且禁止缓存的请求显式查看一个旧密钥；该值只保留在当前页面内存中。
- `{md5}` 使用最终发送给图床的字节计算小写 MD5，与 PicFast PicGo 的内容 MD5 算法一致；启用图片处理时以处理后的字节为准，扩展名由已验证的 MIME 类型决定。
...
- 本地图片粘贴和拖放通过受保护的同源图床路径上传到管理员配置的 Cloudflare R2、七牛云 Kodo、阿里云 OSS 或腾讯云 COS；任一服务均可作为主图床或可选备份图床，主备写入固定使用同一个对象 Key，主配置的查看图片域名负责生成唯一返回 URL。一个 `0` 至 `5` 的额外重试次数同时用于主图床和已启用的备份图床串行写入；任一必需目标耗尽尝试后整次上传失败且不插入 URL，也不会切换服务或回退到 WordPress 媒体库。
- 图床设置支持使用内置 EasyMDE PNG 执行真实验证上传，并用结构化消息弹窗显示实际对象路径、文章图片 URL 及必要的 HTTP 混合内容风险提示。
- 本地草稿恢复和独立沉浸写作工作台。
...
- 配置中心默认把编辑器中选择的外观应用到发布内容；关闭联动后，所选外观仅用于编辑器预览，公开内容使用 EasyMDE 的中性默认外观。
- 支持 CSS-only 的 Mac 风格代码块边框。
- 发布文章的代码块默认显示复制按钮；可在配置中心关闭该按钮，且不会影响代码渲染和语法高亮。
