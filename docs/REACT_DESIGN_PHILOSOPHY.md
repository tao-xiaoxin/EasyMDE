...
国际化不是单个 React Helper，而是 Source Message、Translation Owner、Extraction、Catalog、WordPress Loading、Locale / RTL、Accessibility Copy、Release Package 和 Runtime Evidence 的一条端到端合同。一个用户可见消息实例只有一个 Translation Owner；不得同时由 PHP Bootstrap 与 JavaScript Catalog 翻译，也不得把 Error Code、ID、Route、Handle、Storage Key 或扩展标识符当作显示文案翻译。
...
具体的 PHP Gettext、React Message、提取、JSON Filename、Script Translation、格式化、RTL、测试和发布包执行规则由 `.agents/skills/i18n/SKILL.md` 唯一负责；本文只保留长期所有权与交付边界。
...
本地、版本控制内的 Runtime Asset 是默认方案，因为它最容易保证发布可复现、隐私、离线或受限网络下的可用性、依赖审计和故障边界。默认选择本地不等于把“远程”本身视为缺陷；真正需要拒绝的是来源不明、内容可变、隐私侵入、不可验证或把核心能力交给未审查单点故障的资源。
...
- 排除 Dev Server URL、Localhost、Source Path、未批准 Source Map、未批准远程资源和 Development Code；
- HMR / Fast Refresh 只用于开发体验，Full Reload、重复 Mount / Unmount 和 Production Build 必须同样正确。

依赖必须有当前 Owner、非重复责任、可接受 License、直接和传递体积、活跃维护、无未批准 Telemetry / Remote Runtime、测试、移除策略、Lockfile 更新和 Third-party Notice。
...
15. Root-relative Plugin Asset URL、未批准、可变、非官方、跟踪或隐私侵入的 Remote Runtime Asset、Production Dev Server Reference 或未批准 Telemetry；
